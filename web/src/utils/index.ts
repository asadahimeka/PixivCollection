...
export function getImageMediumSrc(store: any, img: Image) {
  if (!store.masonryConfig.useLocalImage || !imgDir) {
    return img.images?.m.replace('i.pximg.net', 'pximg.cocomi.eu.org') || `https://f.pixiv.pics/pid/${img.id}?size=medium`
  }
...
export function getImageLargeSrc(store: any, img: Image) {
  if (!store.masonryConfig.useLocalImage || !imgDir) {
    return img.images?.l.replace('i.pximg.net', 'pximg.cocomi.eu.org').replace(/\/c\/\d+x\d+_\d+(_webp)?\//, '/') || `https://f.pixiv.pics/pid/${img.id}?size=large&p=${img.part}`
...
export function getImageOriginalSrc(img: Image) {
  return img.images?.o.replace('i.pximg.net', 'pximg.cocomi.eu.org') || `https://f.pixiv.pics/pid/${img.id}?p=${img.part}`
}
