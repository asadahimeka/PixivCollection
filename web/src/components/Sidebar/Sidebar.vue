...
const filteredAuthors = computed(() => {
  if (searchAuthor.value !== '') {
    ...
    if (showFullAuthor.value) { return authors.value } else { return authors.value.slice(0, 20) }
  }
})
const filteredTags = computed(() => {
  if (searchTag.value !== '') {
    ...
    if (showFullTags.value) { return tags.value } else { return tags.value.slice(0, 10) }
  }
})
...
function handleChangeR18(event: Event) {
  ...
  if (target.value === 'show' || target.value === 'only') { filterConfig.value.restrict.maxSanityLevel = 6 }
}
